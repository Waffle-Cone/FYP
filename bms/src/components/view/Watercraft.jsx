...
function WaterCraft() {
  ...
  return (
    <>
      {!showModal ? null : MODAL.DeleteConfirm(showModal, handleModalClose, toDelete, handleDelete, "watercraft")}
        ...
        <>
          <CardContainer>
            {watercrafts.map((watercraft) => (
              <WatercraftCard editMode={editMode} openModal={openModal} watercraft={watercraft} key={watercraft.Synthetic_Key} />
            ))}
          </CardContainer>
        </>
      )}
